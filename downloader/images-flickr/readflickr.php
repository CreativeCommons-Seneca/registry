...
if ($myfile) {
    ...
    //Get file contents
      ...
      // If it is error file, download the image again.
		...
		// If duplicate, move to the next file
		if ($rows > 0) {
			...
			// Convert to jpg if the image is png
	    ...
	    // Try hashing the image 
			try{
				$hash = exec('./phash '.$imagefile);

				// If hashed successfully, resize to 300 and save to directory
				if(strcmp($hash,"ph_dct_imageash returned < 0") === true){
					...
					try{
						$str = 'convert '.escapeshellarg($imagefile).' -resize 300 '.escapeshellarg($dir."/".$name);
						$thumb = exec($str);
					}catch (Exception $e){
						echo "\nRESIZE ERROR\n".$imagefile;
					}
			...
			// If empty fields, record the message
          ...
          // MYSql Insert Statement
					...
					echo "\n\nDB ERROR!! ".$e;
				} // close try and catch insert into DB
			}
