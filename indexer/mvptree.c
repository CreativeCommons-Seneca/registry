...
static MVPError _mvptree_print(FILE *stream, MVPTree *tree, Node *node, int lvl){
	...
	if (next_node->leaf.type == LEAF_NODE){
		...
		fprintf(stream, "        point[%d]: %s\t", i, next_node->leaf.points[i]->id);
		int len;
		for (len = 0; len < next_node->leaf.points[i]->datalen; len++)
	        fprintf(stream,"%02x", ((uint8_t *)next_node->leaf.points[i]->data)[len]);
		fprintf(stream, "\n");
	    }
